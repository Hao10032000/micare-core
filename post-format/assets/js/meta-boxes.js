(function( $ ) {
    ...
    function togglePostFormatMetaBoxes() {
        var $input = $('input[name=post_format],input[name*="post-format-selector-'),
        ...
        $('.components-button').each(function(){
            ...
            if ($(this).text().toLowerCase() == 'gallery' || $(this).text().toLowerCase() == 'video' || $(this).text().toLowerCase() == 'audio' || $(this).text().toLowerCase() == 'quote') {
                $('[id*="themesflat-options-control-' + $(this).text().toLowerCase() + '"]').show();
            } else $('#themesflat-options-control-blog_heading').show();
        })

        if ($('.editor-post-panel__row .editor-post-panel__row-control')) {
            $input.trigger('change');
        }
        //Gutenberg
        jQuery(document).on('change', 'select[id*="post-format"],input[name*="post-format-selector-"]',function(){             
            $metaBoxes.hide();
            if ($(this).val() == 'gallery' || $(this).val() == 'video' || $(this).val() == 'audio' || $(this).val() == 'quote') {
                $('[id*="themesflat-options-control-' + $(this).val() + '"]').show();
            } else $('#themesflat-options-control-blog_heading').show(); 
            ...
            frame.on( 'select', function() {                  
                ...
                var images = frame.state().get("selection").models;
...

                    {
                        ...
                        var image_url = images[iii].changed.title;

                        imgContainer.append( '<li><h4> '+image_url+' </h4> <a href="#" id="'+images[iii].id+'" class="themesflat-remove-media" title="Remove"> <span class="dashicons dashicons-no-alt">x</span> </a>' );
