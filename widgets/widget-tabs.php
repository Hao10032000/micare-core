...
class TFTabs_Widget extends \Elementor\Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'TF Tabs Core', 'themesflat-core' );
    }
	...
	protected function register_controls() {
        // Start Tab Setting        
			...
			$this->add_control( 'show_toggle',
				[
					'label' => esc_html__( 'Tab With Navigation Toggle', 'themesflat-core' ),
					'type' => \Elementor\Controls_Manager::SWITCHER,
					'label_on' => esc_html__( 'Show', 'themesflat-core' ),
					'label_off' => esc_html__( 'Hide', 'themesflat-core' ),
					'description'	=> 'Only Show 2 Content',
					'return_value' => 'yes',
					'default' => 'no',
				]
			); 
			...
			$this->add_control(
				'text_before',
				[
					'label' => esc_html__( 'Text Before', 'themesflat-core' ),
			...
			$this->add_control(
				'text_after',
				[
					'label' => esc_html__( 'Text After', 'themesflat-core' ),
					'type' => \Elementor\Controls_Manager::TEXT,
					'default' => 'Years (save 10%)',
					'condition' => [
						'show_toggle' => 'yes',
					]
				]
	        ...
	        $this->add_control( 'tab_list',
				[					
					...
					'default' => [
						[
							'list_title' => esc_html__( 'About Us', 'themesflat-core' ),
						...
						[
							'list_title' => esc_html__( 'Mission', 'themesflat-core' ),
							'list_content' => esc_html__( 'set it amet consectetur adipiscing elit. posuere eu accumsan placerat et tempus, ac pellentesque. vitae tellus fringilla ullamcorper tempus nullam nulla. amet nec tristique aliquet tristique dui. gravida tellus', 'themesflat-core' ),
						],
						[
							'list_title' => esc_html__( 'Vision', 'themesflat-core' ),
							'list_content' => esc_html__( 'set it amet consectetur adipiscing elit. posuere eu accumsan placerat et tempus, ac pellentesque. vitae tellus fringilla ullamcorper tempus nullam nulla. amet nec tristique aliquet tristique dui. gravida tellus', 'themesflat-core' ),
						],
					],
		...
		$this->start_controls_section( 'section_style_toggle',
			[
				'label' => esc_html__( 'Tab Toggle', 'themesflat-core' ),
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .tf-tabs .toggle-btn-over' => 'justify-content: {{VALUE}};-webkit-justify-content:{{VALUE}};',
				],
			]
		);
		...
		$this->add_control( 
			'size_toggle',
			[
				'label' => esc_html__( 'Size Toggle', 'themesflat-core' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => 'big',
				'options' => [
					'big' => esc_html__( 'Big', 'themesflat-core' ),
					'small' => esc_html__( 'Small', 'themesflat-core' ),
				],
			]
		);
		...
		$this->add_responsive_control( 
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .tf-tabs .toggle-btn-over' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],					
			]
		);
		...
		$this->add_responsive_control( 
			...
			[
				...
				'type' => \Elementor\Controls_Manager::DIMENSIONS,
				'selectors' => [
					'{{WRAPPER}} .tf-tabs .toggle-btn-over' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],					
			]
		);
		...
		$this->add_responsive_control( 
			...
			[
				...
				'size_units' => [ 'px' , '%' ],
				'selectors' => [
					'{{WRAPPER}} .tf-tabs .toggle-btn-tabs' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		); 
		...
		$this->add_control( 
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .tf-tabs .toggle-btn-tabs' => 'background: {{VALUE}}',
				],
			]
		); 

		$this->add_control( 
			'background_color_toggle_hover',
			[
				'label' => esc_html__( 'Background Color Active', 'themesflat-core' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .tf-tabs .toggle-btn-tabs.active' => 'background: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			'h_toggle_inner',
			[
				'label' => esc_html__( 'Inner Toggle', 'themesflat-core' ),
				'type' => \Elementor\Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
		...
		$this->add_responsive_control( 
			'border_radius_toggle_inner',
			[
				'label' => esc_html__( 'Border Radius', 'themesflat-core' ),
				'type' => \Elementor\Controls_Manager::DIMENSIONS,
				'size_units' => [ 'px' , '%' ],
				'selectors' => [
					'{{WRAPPER}} .toggle-btn-tabs .btn-tog.btn-1::after' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		); 

		$this->add_control( 
			'background_color_toggle_inner',
			[
				'label' => esc_html__( 'Background Color', 'themesflat-core' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .toggle-btn-tabs .btn-tog.btn-1::after' => 'background: {{VALUE}}',
				],
			]
		); 
		...
		$this->add_control(
			'h_toggle_text',
			[
				'label' => esc_html__( 'Text Toggle', 'themesflat-core' ),
				'type' => \Elementor\Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
		...
		$this->add_group_control( 
			...
			[
				'name' => 'typography_toggle',
				'label' => esc_html__( 'Typography', 'themesflat-core' ),
				'selector' => '{{WRAPPER}} .tf-tabs .toggle-btn-over .text-toggle',
			]
		); 

		$this->add_control( 
			'typography_toggle_color_toggle',
			[
				'label' => esc_html__( 'Color', 'themesflat-core' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'default' => '',
				'selectors' => [
					'{{WRAPPER}}  .tf-tabs .toggle-btn-over .text-toggle' => 'color: {{VALUE}}',
				],
			]
		); 
		...
		$this->add_responsive_control( 
			...
			[
				...
				'type' => \Elementor\Controls_Manager::DIMENSIONS,
				'selectors' => [
					'{{WRAPPER}} .tf-tabs .toggle-btn-over .text-toggle' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],					
			]
		);
		...
		$this->add_responsive_control( 
			...
			[
				...
				'type' => \Elementor\Controls_Manager::DIMENSIONS,
				'selectors' => [
					'{{WRAPPER}} .tf-tabs .toggle-btn-over .text-toggle' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],					
			]
		);
	    ...
	    // Start Style Tab Nav
			...
			$this->add_responsive_control( 'nav_align',
				[
					...
					'selectors' => [
						'{{WRAPPER}} .tf-tabs .tf-tabnav' => 'text-align: {{VALUE}}',
					],
					'condition' => [
                        'tab_type' => 'horizontal',
                    ],
			...
			$this->add_group_control( 
				...
				[
					...
					'selector' => '{{WRAPPER}} .tf-tabs .tf-tabnav ul',
				]
			);
	        ...
	        $this->start_controls_tabs( 'title_style_tabs' );
	        	$this->start_controls_tab( 'title_style_normal_tab',
					...
					$this->add_control( 'titlebg_color',
						[
							'label' => esc_html__( 'Color', 'themesflat-core' ),
							'type' => \Elementor\Controls_Manager::COLOR,
							'selectors' => [
				...
				$this->start_controls_tab( 'title_style_hover_tab',
					...
					$this->add_control( 'title_color_hover',
						[
							...
							'type' => \Elementor\Controls_Manager::COLOR,
							'selectors' => [
								'{{WRAPPER}} .tf-tabs .tf-tabnav ul > li:hover .tab-title-text, {{WRAPPER}} .tf-tabs .tf-tabnav ul > li.active .tab-title-text
								, {{WRAPPER}} .tf-tabs .tf-tabnav ul > li.set-active-tab .tab-title-text' => 'color: {{VALUE}};',
							],
						]
					);

					$this->add_control( 'titlebg_color_hover',
						[
							'label' => esc_html__( 'Color', 'themesflat-core' ),
							'type' => \Elementor\Controls_Manager::COLOR,
							'selectors' => [
								'{{WRAPPER}} {{WRAPPER}} .tf-tabs .tf-tabnav ul > li::after, {{WRAPPER}} .tf-tabs .tf-tabnav ul > li.active
								, {{WRAPPER}} .tf-tabs .tf-tabnav ul > li.set-active-tab, {{WRAPPER}} .tf-tabs .tf-tabnav ul > li:hover' => 'background-color: {{VALUE}}',
							],
						]
					);
	...
	protected function render($instance = []) {
		...
		$this->add_render_attribute( 'tf_tabs_wrapper', ['id' => "tf-tabs-{$this->get_id()}", 'class' => ['tf-tabs', $settings['tab_type'], 'tabs-'.$settings['tab_type'], $settings['icon_position'], $settings['size_toggle'], 'show-triger-'.$settings['show_triger'] ], 'data-tabid' => $this->get_id()] );
		...
		<div <?php echo $this->get_render_attribute_string('tf_tabs_wrapper'); ?>>
			...
			<?php if ( $settings['show_toggle'] == 'yes' ): ?>
				<div class="toggle-btn-over">
					<?php if ( $settings['text_before'] != '' ): ?>
					<div class="text-toggle left active"><?php echo esc_attr( $settings['text_before'] ); ?></div>
					<?php endif; ?>
					<div class="toggle-btn-tabs">
						<span class="btn-tog btn-1" data-tab="tab-1"></span>
						<span class="btn-tog btn-2" data-tab="tab-2"></span>
					</div>
					<?php if ( $settings['text_after'] != '' ): ?>
					<div class="text-toggle right"><?php echo esc_attr( $settings['text_after'] ); ?></div>
					<?php endif; ?>
				</div>
			<?php else: ?>
			<div class="tf-tabnav">
					...
					<li class="tablinks <?php echo esc_attr($tab['set_active']); ?> elementor-animation-<?php echo esc_attr($settings['title_hover_animation']); ?>" data-tab="tab-<?php echo esc_attr($count_li); ?>">	
						<?php if ( $settings['show_icon'] == 'yes' ) {
							...
							if ( $tab['icon_style'] == 'image' ) {								
								echo '<img src="' . esc_url($tab['image']['url']) . '" class="lazyload" alt="image"/>'; 
							} else {
								if ( $is_new || $migrated ) {
			...
			<div class="tf-tabcontent">
				...
				<div class="tab-<?php echo esc_attr($count_content); ?> tf-tabcontent-inner <?php echo esc_attr($tab['set_active']); ?> animated <?php echo esc_attr($settings['content_entrance_animation']); ?>">
					<?php 
